...
public class RaceCondition {
  ...
  public static void main(String[] args) {
    System.out.println(
        "Main thread with name: " + Thread.currentThread().getName() + " has started");
  ...
  private static void printResult(List<CustomThread> threadList) {
    ...
    freqMap.forEach(
        (k, v) -> freqToListUuidMap.computeIfAbsent(v, val -> new ArrayList<>()).add(k));
    freqToListUuidMap.forEach(
        (k, v) -> System.out.println("Ids generated " + k + " no.of times were: " + v.size()));
  }
        ...
        if (!customThread.isAlive() && !completedThreadSet.contains(customThread.getId())) {
          System.out.println(
              "Thread "
                  ...
                  + " is in state "
                  + customThread.getState()
                  + " trying to join now");
          try {
            customThread.join();
            System.out.println(
                "Thread " + customThread.getName() + " joined! State= " + customThread.getState());
          } catch (InterruptedException e) {
            System.out.println(
                "Caught InterruptedException in main thread! Couldn't join "
                    + customThread.getName());
            failedSize++;
          }
          joinedSize++;
