...
public class Middleware {
  ...
  public synchronized void publish(Message message) throws InterruptedException {
    ...
    while (buffer.size() >= size) {
      ...
      wait();
      System.out.println(
          prefix + " has woken up and is in state " + Thread.currentThread().getState());
    }
    System.out.println(
        prefix + " inserting the message " + message + " in buffer, size: " + buffer.size());
  ...
  public synchronized Message consume() {
    ...
    System.out.println(
        prefix + " notifying consumers if they are blocked *Sigh* Is this my job though :(");
    notifyAll();
    return message;
  }
}
